...
with System.RM46.Pll;      use System.RM46.Pll;
...
--   RTICOMP2     : constant Address := RTI_Base + 16#60#;
--   RTIUDCP2     : constant Address := RTI_Base + 16#64#;
   ...
   RTI_Compare_Interrupt_2 : constant Interrupts.Interrupt_ID := 4;
   --  Interrupt ID for periodic tick on Compare2
   ...
   --  vvvvvvv needs investigation. Why are we 4x rather than 2x per
   --  the comment  below. (HR for the RM46 05_01_15)
   RTI_kHz   : constant Word := Word (Parameters.Clock_Frequency / 4000);
...
--      Write (RTICOMPCTRL, 2**12 or 2**8);          -- CMP2&3 against FRC1
--      Write (RTIINTFLAG, 2**3 or 2**2);
      ...
      --  Add a periodic interrupt also on CMP2
--      Write (RTICOMP2, 50000);                     --  FRC1 + .01sec
--      Write (RTIUDCP2, 50000);                     --  100x/sec tick

--      Write (RTISETINTENA, 2**3 or 2**2);           -- Enable Interrupts

--      Interrupt_Vectors (RTI_Compare_Interrupt_2) := IRQ_Handler'Address;
--      Enable_Interrupt_Request (RTI_Compare_Interrupt_2);
   ...
   function Tick_Interrupt_ID return Interrupts.Interrupt_ID is
      (RTI_Compare_Interrupt_2);

   ---------------------------
   -- Clear_Tick_Interrupt --
   ---------------------------

   procedure Clear_Tick_Interrupt is
   begin
      Write (RTIINTFLAG, 2**2);
   end Clear_Tick_Interrupt;
