...
package body System.BB.CPU_Primitives is
   ...
   --  equal the the Current_FPU_Context. This is on the expectation that the
   --  new context will not use floating point during its execution window.
   --  If it does, then an undefined instruction trap will be executed that
   --  performs the context switch and retries. We also don't restore the
   --  FPU enabled state when leaving an interrupt handler that didn't use
   --  the FPU as we rather incur the trap at the user level than leaving
   --  interrupt masked longer than absolutely necessary.
   ...
   procedure Undef_Handler is
      ...
      In_IRQ_Or_FIQ : Boolean;

   begin
      In_IRQ_Or_FIQ := (SPSR mod 32) in 17 | 18;

      if not Is_FPU_Enabled then
         Set_FPU_Enabled (True);
         FPU_Context_Switch
           (if In_IRQ_Or_FIQ then null else Queues.Running_Thread_Table (1));
      else
   ...
   begin
      ...
      Asm
        (Template =>
         ...
         & "ldr   r2,=0f"        & NL  -- Adjust R2 to point past ctx switch
         & "add   r2,r2,#1"      & NL  -- Force thumb
         ...
         & "msr   CPSR_cxsf, r3" & NL  -- Move user CPSR to our CPSR
         & "bx    r2"            & NL  -- Return to the context
         & ".thumb"              & NL
         & "0:",                       -- Label indicating where to continue
         Inputs   =>
           (Address'Asm_Input ("r", Queues.Running_Thread_Table (1)'Address),
            Thread_Id'Asm_Input ("r", Queues.First_Thread_Table (1))),
         Volatile => True,
         Clobber  => ("memory,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,r12,lr"));
         --  Clobber  => ("memory,r2,r3,r4,r5,r6,lr"));
   ...
   procedure FPU_Context_Switch (To : Thread_Id) is
      C : constant Thread_Id := Current_FPU_Context (1);
